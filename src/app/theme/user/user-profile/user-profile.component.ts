...
export class UserProfileComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.uploaderfirma.onAfterAddingFile = (file: any) => {
      file.withCredentials = false;
    };
    this.uploaderfirma.onSuccessItem = (item, response, status, headers) =>
      this.onSuccessItemFirma(item, response, status, headers);
  ...
  cargardatos() {
    this.userService.getById(this.user.id).subscribe(
      (value: any) => {
        ...
        this.imageUrl =
          this.apiurl + 'upload/files_client/' + this.usuario.firmaOriginal;
        this.noshow = true;
      },
      err => {
  ...
  actualizarItem(item) {
    this.userService.update(item).subscribe(
      value => {
        this.notificationService.addNotify({
          title: 'Alerta',
          msg: 'Uusario actualizado con exito',
          type: 'success'
        });
        item.edit = false;
        this.noshow = true;
  ...
  resultImageFun(event: ImageCropperResult) {
    ...
    this.usuario.firma = this.angularCropper.cropper
      .getCroppedCanvas()
