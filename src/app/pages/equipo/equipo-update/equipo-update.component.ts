...
export class EquipoUpdateComponent implements OnInit {
  ...
  constructor(private notificationService: NotificationService,
    ...
    if (document.domain === 'localhost') {
      this.myAngularxQrCode = `${document.domain}:4200/equipo/public/${this.route.snapshot.paramMap.get('id')}`;
    } else {
      this.myAngularxQrCode = `${document.domain}/app/equipo/public/${this.route.snapshot.paramMap.get('id')}`;
    }
    ...
    try {
      this.InfoAdicionalEquiposService.get().subscribe( (resp: any) => {
        if (resp.success) {
          ...
          this.item.infoAdicionalHeader = this.InfoAdicionalGlogal['infoAdicionalHeader'];
  ...
  ngOnInit() {
    ...
    this.uploaderImagenEditor.onSuccessItem = (item, response, status, headers) => this.onSuccessItemImagenEditor(item, response, status, headers);
    ...
    if (this.item._id == undefined) {
      ...
      this.configuracionesEquipoService.getconfiguracionEquipos().subscribe( (resp: any) => {
        if (resp.success) {
          ...
          if (this.item.infoAdicionalHeader === '' || this.item.infoAdicionalHeader === undefined) {
            this.item.infoAdicionalHeader = this.InfoAdicionalGlogal['infoAdicionalHeader']
          }
          if (this.item.infoAdicional === '' || this.item.infoAdicional === undefined) {
            this.item.infoAdicional = this.InfoAdicionalGlogal['infoAdicional']
          }
  ...
  cargardatos() {
    ...
    this.emails1 = [];
    this.emails2 = [];
    this.equipoService.getById(this.id).subscribe((value) => {
      console.log(value)
  ...
  resultImageFun(event: ImageCropperResult) {
    ...
    this.item.imagen = this.angularCropper.cropper.getCroppedCanvas({
      fillColor: '#fff',
  ...
  calculaVaribleMantenimiento(): void {
    ...
    const cantidadDias = this.formconfiguraciones.value.Rcada * parseInt(this.formconfiguraciones.value.tiempo);
    const cantidadDias2 = this.formconfiguraciones.value.Rcada2 * parseInt(this.formconfiguraciones.value.tiempo2);
    ...
    for (const i in variables) {
      ...
      variables[i].fechaAviso = moment(variables[i].proxcalibracion).subtract(cantidadDias, 'days').format();
    }
    this.item.variables = [...variables];
    // tslint:disable-next-line: forin
    for (const i in reparaciones) {
      reparaciones[i].fechaAviso = moment(reparaciones[i].fechaNovedad).subtract(cantidadDias2, 'days').format();
    }
    this.item.variables = [...variables];
  ...
  calcularEventos() {
    // if (this.formaEventos.valid) {
      ...
      const incremento = parseInt( this.formaEventos.value.intervalo) * parseInt(this.formaEventos.value.repetirCada);
      ...
      let i: number = 0;
        do {
          ...
          if(fechaDif2 >=0 ){
            console.log(`fecha ciclo ${moment(dayInit4).format()} es menor igual a ${moment(fechaFin).format()}  `  );
            
          }else{
            console.log(`fecha ciclo ${moment(dayInit4).format()} es mayor igual a ${moment(fechaFin).format()} se debe cerrar ciclo en la iteracion ${i} `  );
            arrayIntervalos.pop();
          }
          i++;
        } while ( fechaDif2 >= 0)
