...
export class UsuariosService {
  ...
  add(userData) {
    let url = this.environmentService.setApiService('auth/signup');
  ...
  getBytoken($id, email) {
    let url = this.environmentService.setApiServiceById(this.metodo + '/buscartoken', $id);
    url += '?email=' + email;
    return this._http.get(url)
      .map(res => res)
      .catch(this.handleError);
  ...
  acttoken(userData) {
    let url = this.environmentService.setApiServiceById(this.metodo+'/acttoken', userData._id);
    return this._http.put(url, userData)
      .map(res => res)
      .catch(this.handleError);
  }
